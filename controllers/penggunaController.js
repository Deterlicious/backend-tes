...
class PenggunaController {
  ...
  async loginPin(req, res, next) {
    try {
      ...
      res.json({
        ...
        refreshToken: result.refreshToken, // Kirim juga di body untuk client non-browser
        data: {
          id: result.user._id,
  ...
  async create(req, res, next) {
    try {
      // Logic deteksi apakah ini Public Registration (Owner) atau Admin Add Staff
      // Jika req.pengguna ada, berarti admin yang nambah. Jika tidak, public register.
      const isSelfRegister = !req.pengguna; 

      const result = await penggunaService.create(req.body, isSelfRegister);
  ...
  async getById(req, res, next) {
    try {
      const result = await penggunaService.getById(req.params.id);
  ...
  async update(req, res, next) {
    try {
      const result = await penggunaService.update(req.params.id, req.body);
  ...
  async delete(req, res, next) {
    try {
      const result = await penggunaService.delete(req.params.id);
      if (!result) throw createError(404, "User tidak ditemukan");
