...
class TransferStokController {
  ...
  async createTransferStok(req, res, next) {
    try {
      const payload = {
        ...req.body,
  ...
  async getAllTransferStok(req, res, next) {
    try {
  ...
  // READ BY ID
  async getTransferStokById(req, res, next) {
    try {
      const { id } = req.params;
      const { tenantID } = req.pengguna;
  ...
  async updateTransferDraft(req, res, next) {
    try {
      const { tenantID } = req.pengguna;
      const { id } = req.params;
  ...
  async markAsKirim(req, res, next) {
    try {
  ...
  async markAsTerima(req, res, next) {
    try {
      const { tenantID, _id: userID } = req.pengguna;
      const { id } = req.params;
  ...
  // UPDATE STATUS: BATAL
  async markAsBatal(req, res, next) {
    try {
      const { tenantID, _id: userID } = req.pengguna;
      const { id } = req.params;
  ...
  // DELETE DRAFT
  async deleteTransferDraft(req, res, next) {
    try {
      const { tenantID } = req.pengguna;
      const { id } = req.params;
