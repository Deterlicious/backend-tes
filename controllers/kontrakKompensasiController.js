...
class KontrakKompensasiController {
  ...
  async getById(req, res, next) {
    try {
      const tenantID = this._getTenantID(req);
      const result = await kontrakKompensasiService.getById(
        req.params.id,
        tenantID,
      );
  ...
  async update(req, res, next) {
    try {
      const tenantID = this._getTenantID(req);
      const result = await kontrakKompensasiService.update(
        req.params.id,
        req.body,
        tenantID,
      );
  ...
  async delete(req, res, next) {
    try {
      const tenantID = this._getTenantID(req);
      const result = await kontrakKompensasiService.delete(
        req.params.id,
        tenantID,
      );
