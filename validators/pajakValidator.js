...
const VALID_MODEL = [1, 2, 3]; // 1=Inclusive, 2=Exclusive, 3=Compound
...
function validatePajakPayload(data, isUpdate = false) {
  ...
  if (!isUpdate) {
    ...
    if (!data.namaPajak || (data.namaPajak + "").trim().length === 0) {
    ...
    if (!data.prioritas || !VALID_PRIORITAS.includes(data.prioritas)) {
  ...
  // Shared validations (create & update)
  ...
  if (data.tipePajak && !VALID_TIPE.includes(data.tipePajak)) {
