...
function validatePenjualanPayload(data, isUpdate = false) {
  ...
  if (!isUpdate) {
    ...
    if (!data.itemPenjualan || !Array.isArray(data.itemPenjualan) || data.itemPenjualan.length === 0) {
    ...
    if (data.tanggalPenjualan && isNaN(new Date(data.tanggalPenjualan).getTime())) {
