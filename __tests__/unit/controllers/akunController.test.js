...
describe("Unit Test — Akun Controller", () => {
  ...
  describe("Fungsi getProfile()", () => {
    test("Harus mengambil profile berdasarkan ID dari token (req.akun.id)", async () => {
      const req = mockRequest({ userDecoded: { tenantID: "tenant_123" } });
      const res = mockResponse();
      ...
      expect(akunService.getProfile).toHaveBeenCalledWith("tenant_123");
      expect(res.json).toHaveBeenCalledWith({
    ...
    test("Harus mengembalikan 401 jika tenantID tidak ada di userDecoded (Guard Internal)", async () => {
      const req = mockRequest({ userDecoded: {} }); // tenantID sengaja dikosongkan
      const res = mockResponse();
      ...
      expect(akunService.getProfile).not.toHaveBeenCalled(); // Service tidak boleh tersentuh
    });
  });
  ...
  describe("Fungsi updateProfile()", () => {
    test("Harus meneruskan req.akun.id dan req.body ke service", async () => {
      const req = mockRequest({
        userDecoded: { tenantID: "tenant_123" },
      ...
      expect(akunService.updateProfile).toHaveBeenCalledWith(
        "tenant_123",
        req.body,
      );
      expect(res.json).toHaveBeenCalledWith(
    ...
    test("Harus mengembalikan 403 jika tenantID tidak ada di userDecoded (Guard Internal)", async () => {
      const req = mockRequest({ userDecoded: {} }); // tenantID sengaja dikosongkan
      const res = mockResponse();
      ...
      expect(akunService.updateProfile).not.toHaveBeenCalled(); // Service tidak boleh tersentuh
    });
  });
  ...
  describe("Fungsi logout()", () => {
    test("Harus memanggil service logout dan membersihkan cookie", async () => {
      ...
      expect(akunService.logout).toHaveBeenCalledWith("token_123", null);
    ...
    test("Harus bisa mengambil token dari body jika cookie kosong", async () => {
      ...
      expect(akunService.logout).toHaveBeenCalledWith("token_body_123", null);
    });

    test("Harus mengekstrak accessToken dari header Authorization Bearer", async () => {
      const req = mockRequest({
        cookies: { refreshToken: "refresh_123" },
        headers: { authorization: "Bearer access_123" }, // Simulasi token di header
      });
      const res = mockResponse();
      ...
      // Pastikan service dipanggil dengan DUA argumen
      expect(akunService.logout).toHaveBeenCalledWith(
        "refresh_123",
        "access_123",
      );
    });
  ...
  describe("Rute Admin (getAllAkun & deleteUserByAdmin)", () => {
    test("getAllAkun: Harus mengembalikan list user", async () => {
      const req = mockRequest({ userDecoded: { id: "admin_123" } });
      const res = mockResponse();
      ...
      expect(akunService.getAllAkun).toHaveBeenCalledWith("admin_123");
      expect(res.json).toHaveBeenCalledWith({
    ...
    test("deleteUserByAdmin: Harus meneruskan req.params.id ke service", async () => {
      const req = mockRequest({
        params: { id: "target_123" },
        userDecoded: { id: "admin_123" },
      });
      const res = mockResponse();

      akunService.deleteUserByAdmin.mockResolvedValue(true);

      await akunController.deleteUserByAdmin(req, res, mockNext);

      expect(akunService.deleteUserByAdmin).toHaveBeenCalledWith(
        "target_123",
        "admin_123",
      );
      expect(res.json).toHaveBeenCalledWith({
    ...
    test("Harus mengembalikan 401 jika requesterId tidak ada di userDecoded (Guard Internal)", async () => {
      const req = mockRequest({ userDecoded: {} }); // id sengaja dikosongkan
      const res = mockResponse();
      ...
      // Pastikan kedua service admin tidak dipanggil
      expect(akunService.getAllAkun).not.toHaveBeenCalled();
      expect(akunService.deleteUserByAdmin).not.toHaveBeenCalled();
    });
  });
  ...
  describe("Fungsi refreshToken() (Jalur yang Terlupakan)", () => {
    ...
    test("Harus mengambil token dari body jika cookie kosong", async () => {
      ...
      const res = mockResponse();
      akunService.refreshToken.mockResolvedValue({
        accessToken: "a",
        refreshToken: "b",
      });
  ...
  describe("Keandalan Error Handling (Propagasi ke next)", () => {
    ...
    test("getProfile: Harus melempar error ke next()", async () => {
      const req = mockRequest({ userDecoded: { tenantID: "1" } });
      const res = mockResponse();
    ...
    test("updateProfile: Harus melempar error ke next()", async () => {
      const req = mockRequest({ userDecoded: { tenantID: "1" }, body: {} });
      const res = mockResponse();
    ...
    test("Rute Admin: Harus melempar error ke next()", async () => {
      ...
      akunService.deleteUserByAdmin.mockRejectedValue(errorSistem);
      await akunController.deleteUserByAdmin(req, res, mockNext);
      expect(mockNext).toHaveBeenCalledWith(errorSistem);
    });

    test("refreshToken: Harus menghancurkan cookie sebelum melempar error ke next()", async () => {
      const req = mockRequest({ cookies: { refreshToken: "bad_token" } });
      const res = mockResponse();
      const error = new Error("Token Kedaluwarsa");

      akunService.refreshToken.mockRejectedValue(error);

      await akunController.refreshToken(req, res, mockNext);

      // Pastikan res.cookie dipanggil untuk menghancurkan cookie (maxAge: 0)
      expect(res.cookie).toHaveBeenCalledWith(
        "refreshToken",
        "",
        expect.objectContaining({ maxAge: 0 }),
      );
      // Pastikan error tetap diteruskan ke middleware express
      expect(mockNext).toHaveBeenCalledWith(error);
    });
  });
