...
describe("Unit Test Tenant Validator", () => {
  describe("Mode Create (isUpdate = false)", () => {
    test("harus valid meskipun field status tidak dikirim sama sekali", () => {
      // Pembuktian bahwa status bersifat opsional di validator
      const payload = { namaToko: "Toko Maju" };
  ...
  describe("Mode Update (isUpdate = true)", () => {
    test("harus valid meskipun payload dikirim kosong melompong (ekstrem partial update)", () => {
      const payload = {}; // Klien tidak mengubah apa-apa
  ...
  describe("Validasi Status dan Multi-Error", () => {
    test("harus gagal jika status tidak dikenali", () => {
      ...
      expect(result.errors).toContain(
        "status hanya boleh 'aktif' atau 'non-aktif'",
      );
    });
